...
// Largura mínima do grid para caber todas as colunas sem apertar demais.
// Em telas menores, habilita scroll horizontal.
...
const MOCK_ALUNOS: Aluno[] = [
  {
    ...
    plano: "Mensal",
    status: "Ativo",
    proximaSessao: new Date(Date.now() + 86400000).toISOString(),
    progresso: 62,
  ...
  },
  {
    ...
    plano: "Anual",
    status: "Ativo",
    proximaSessao: new Date(Date.now() + 3 * 86400000).toISOString(),
    progresso: 85,
  ...
  },
  {
    ...
    plano: "Mensal",
    status: "Ativo",
    proximaSessao: new Date(Date.now() + 2 * 86400000).toISOString(),
    progresso: 47,
  ...
  return (
    <Pressable
      ...
      className={`px-2.5 py-1.5 rounded-xl border ${
        active ? "bg-emerald-500/20 border-emerald-400/30" : "bg-white/5 border-white/10"
      }`}
    >
      <Text
        allowFontScaling={false}
        className={`${active ? "text-emerald-200" : "text-slate-300"} text-[11px] font-medium`}
      >
        {label}
      </Text>
    </Pressable>
...
// Cabeçalho da DataGrid (compacto + ícone válido Feather)
function GridHeader({
  onSort,
  sortKey,
  sortDir,
  ...
  const HeaderCell = ({
    ...
    const iconName: React.ComponentProps<typeof Feather>["name"] = !colKey
      ? "help-circle"
      : isActive
      ? sortDir === "asc"
        ? "chevron-up"
    ...
    return (
      ...
      >
        <Text
          allowFontScaling={false}
          className="text-[10px] uppercase tracking-wider text-slate-400 mr-1"
        >
          {label}
        </Text>
  ...
  return (
    <View
      ...
      className="flex-row items-center border-b border-white/10 px-3 py-2 bg-white/5"
    >
...
function GridRow({ item, onPress }: { item: Aluno; onPress?: (aluno: Aluno) => void }) {
  return (
    <Pressable
      ...
      style={{ width: GRID_WIDTH }}
      className="flex-row items-center px-3 py-2.5 border-b border-white/5"
    >
      <View style={{ flex: 2 }}>
        <Text allowFontScaling={false} className="text-white font-medium text-[13px]" numberOfLines={1}>
          {item.nome}
        </Text>
        {item.email && (
          <Text allowFontScaling={false} className="text-slate-400 text-[11px]" numberOfLines={1}>
            {item.email}
          </Text>
        )}
      </View>
      <View style={{ flex: 1 }} className="items-start">
        <Text
          allowFontScaling={false}
          className={`text-[10px] px-2 py-0.5 rounded-full border ${planoColors[item.plano]}`}
        >
          {item.plano}
        </Text>
      </View>
      <View style={{ flex: 1 }} className="items-start">
        <Text
          allowFontScaling={false}
          className={`text-[10px] px-2 py-0.5 rounded-full border ${badgeClassesByStatus[item.status]}`}
        >
          {item.status}
        </Text>
      </View>
      <View style={{ flex: 1 }}>
        <Text allowFontScaling={false} className="text-slate-200 text-[12px]" numberOfLines={1}>
          {formatDate(item.proximaSessao)}
        </Text>
      </View>
      <View style={{ flex: 1 }}>
...
export default function StudentsScreen() {
  ...
  const [objetivoAtivo, setObjetivoAtivo] =
    useState<Aluno["objetivo"] | "Todos">("Todos");
  ...
  const alunosFiltrados = useMemo(() => {
    let base = dados.filter((a) => {
      ...
      const byBusca =
        ...
        (a.email?.toLowerCase().includes(b) ?? false);
  ...
  return (
    <SafeAreaView className="flex-1 bg-slate-950">
      ...
      <View className="px-4 pt-2 pb-3 border-b border-white/10">
        <Text allowFontScaling={false} className="text-white text-xl font-extrabold">
          Alunos
        </Text>
        <Text allowFontScaling={false} className="text-slate-400 mt-0.5 text-[12px]">
        ...
        <View className="mt-2 gap-2">
          <View className="flex-row flex-wrap gap-2">
            {(["Todos", "Ativo", "Inativo", "Pausado"] as const).map((s) => (
              <Chip
                key={s}
                label={`Status: ${s}`}
                active={statusAtivo === s}
                onPress={() => setStatusAtivo(s as any)}
              />
            ))}
          </View>
          <View className="flex-row flex-wrap gap-2">
            {(["Todos", "Mensal", "Trimestral", "Semestral", "Anual"] as const).map(
              (p) => (
                <Chip
                  key={p}
                  label={`Plano: ${p}`}
                  active={planoAtivo === p}
                  onPress={() => setPlanoAtivo(p as any)}
                />
              )
            )}
          </View>
          <View className="flex-row flex-wrap gap-2">
            {(
              ...
              <Chip
                key={o}
                label={`Objetivo: ${o}`}
                active={objetivoAtivo === o}
                onPress={() => setObjetivoAtivo(o as any)}
              />
            ))}
          </View>
          ...
          <View className="flex-row items-center justify-between mt-1">
            <Pressable onPress={limparFiltros} disabled={!temFiltro} className="px-1.5 py-0.5">
              <Text
                ...
                className={`text-[11px] font-medium ${
                  temFiltro ? "text-slate-300" : "text-slate-600"
                }`}
              >
                Limpar filtros
              </Text>
            ...
            <Pressable
              onPress={() => onSort(sortKey ?? "nome")}
              className="flex-row items-center gap-1 px-1.5 py-0.5"
            >
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          <View>
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <GridRow item={item} onPress={(a) => { /* navegue para detalhes */ }} />
              )}
              ListEmptyComponent={
                <View className="items-center py-8" style={{ width: GRID_WIDTH }}>
                  <Text allowFontScaling={false} className="text-white font-medium text-[13px]">
                    Nenhum aluno encontrado
                  </Text>
                  <Text allowFontScaling={false} className="text-slate-400 text-[12px] mt-1">
